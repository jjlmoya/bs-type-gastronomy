<?php
/**
 * Plugin Name: Gastronomy Model [Post Type]
 * Plugin URI: https://www.bonseo.es/
 * Description: Modelo de Gastronomía
 * Author: jjlmoya
...
require_once plugin_dir_path(__FILE__) . '/Gastronomy.php';
function bs_gastronomy_get_post_type()
{
	return Gastronomy::getInstance('Gastronomía', 'Gastronomías', "gastronomia",
		array()
	);
}

function bs_gastronomy_register_post_type()
{
	$model = bs_gastronomy_get_post_type();
	$labels = array(
...
function bs_gastronomy_create_custom_params()
{
	$model = bs_gastronomy_get_post_type();
...
function bs_gastronomy_register($customType)
{
	$model = bs_gastronomy_get_post_type();
...
function bs_gastronomy_callback($fieldType)
{
	$model = bs_gastronomy_get_post_type();
	$customField = $model->customFields;
...
function bs_gastronomy_on_save($post_id)
{
